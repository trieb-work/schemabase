...
let AppExtensionTargetEnum;

(function (AppExtensionTargetEnum) {
  AppExtensionTargetEnum["Create"] = "CREATE";
  AppExtensionTargetEnum["MoreActions"] = "MORE_ACTIONS";
})(AppExtensionTargetEnum || (AppExtensionTargetEnum = {}));

let AppExtensionTypeEnum;

(function (AppExtensionTypeEnum) {
  AppExtensionTypeEnum["Details"] = "DETAILS";
  AppExtensionTypeEnum["Overview"] = "OVERVIEW";
})(AppExtensionTypeEnum || (AppExtensionTypeEnum = {}));

let AppExtensionViewEnum;

(function (AppExtensionViewEnum) {
  AppExtensionViewEnum["Product"] = "PRODUCT";
})(AppExtensionViewEnum || (AppExtensionViewEnum = {}));
...
let ConfigurationTypeFieldEnum;
...
let ExternalNotificationErrorCodes;
...
let PostalCodeRuleInclusionTypeEnum;
...
let WarehouseClickAndCollectOptionEnum;
...
let WebhookSampleEventTypeEnum;
...
const AppInstallDocument = (external_graphql_tag_default())`
...
`;
const AppTokenVerifyDocument = (external_graphql_tag_default())`
...
`;
const CategoryCreateDocument = (external_graphql_tag_default())`
...
`;
const ChannelCreateDocument = (external_graphql_tag_default())`
...
`;
const ProductChannelListingUpdateDocument = (external_graphql_tag_default())`
...
`;
const ProductCreateDocument = (external_graphql_tag_default())`
...
`;
const ProductTypeCreateDocument = (external_graphql_tag_default())`
...
`;
const ProductVariantChannelListingUpdateDocument = (external_graphql_tag_default())`
...
`;
const ProductVariantCreateDocument = (external_graphql_tag_default())`
...
`;
const TokenCreateDocument = (external_graphql_tag_default())`
...
`;
const WebhookCreateDocument = (external_graphql_tag_default())`
...
`;
const AppDocument = (external_graphql_tag_default())`
  query app($id: ID) {
    app(id: $id) {
      id
      webhooks {
...
`;
const ProductsDocument = (external_graphql_tag_default())`
...
var external_graphql_request_ = __webpack_require__(435);
// EXTERNAL MODULE: ../../libs/util/constants/src/lib/headers.ts
var headers = __webpack_require__(347);
;// CONCATENATED MODULE: ../../libs/adapters/saleor/api/src/lib/client.ts



function createSaleorClient({
  traceId,
  graphqlEndpoint,
  token
}) {
  async function requester(doc, vars) {
    const graphqlClient = new external_graphql_request_.GraphQLClient(graphqlEndpoint);
...
;// CONCATENATED MODULE: ../../libs/adapters/saleor/api/src/index.ts
...
// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  "Zz": function() { return /* reexport */ authorizeIntegration; },
  "sj": function() { return /* reexport */ extendContext; },
  "Vy": function() { return /* reexport */ newSaleorClient; },
  "o7": function() { return /* reexport */ setupPrisma; }
});

;// CONCATENATED MODULE: ../../libs/context/src/lib/context.ts
/**
 ...
 */
async function extendContext(ctx, // eslint-disable-next-line @typescript-eslint/no-explicit-any
...extendContext) {
  if (extendContext) {
...
;// CONCATENATED MODULE: ../../libs/data-access/prisma/src/index.ts

// EXTERNAL MODULE: external "@chronark/env"
var env_ = __webpack_require__(655);
;// CONCATENATED MODULE: ../../libs/context/src/lib/setup/prisma.ts
...
const setupPrisma = () => async ctx => {
  const url = env_.env.require("DATABASE_URL_POOL");
...
var src = __webpack_require__(928);
// EXTERNAL MODULE: ../../libs/adapters/saleor/api/src/index.ts + 2 modules
var api_src = __webpack_require__(799);
;// CONCATENATED MODULE: ../../libs/context/src/lib/setup/saleor.ts
...
const newSaleorClient = (ctx, domain, token) => {
  if (!ctx.prisma) {
    throw new src/* ContextMissingFieldError */.fs("prisma");
  }
  ...
  if (!domain.startsWith("http")) {
    const protocol = env_.env.get("NODE_ENV") === "production" && !env_.env.get("CI") ? "https" : "http";
    domain = `${protocol}://${domain}`;
  }

  return (0,api_src/* createSaleorClient */.NA)({
    traceId: ctx.trace.id,
    graphqlEndpoint: `${domain}/graphql/`,
    token
  });
};
;// CONCATENATED MODULE: ../../libs/context/src/lib/authorizeIntegration.ts

function authorizeIntegration(integration) {
  if (!integration.enabled) {
    throw new src/* HttpError */.oo(403, "The integration is disabled by the user");
  }
  ...
  if (!(subscription !== null && subscription !== void 0 && subscription.payedUntil) || subscription.payedUntil.getTime() < Date.now()) {
    throw new src/* HttpError */.oo(403, "Active subcription required");
  }
}
;// CONCATENATED MODULE: ../../libs/context/src/index.ts
