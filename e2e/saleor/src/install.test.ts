    ...
    it("should create a new saleor app using the manifest route", async () => {
        ...
        await client.appInstall({
            input: {
                ...
                permissions: [
                    PermissionEnum.ManageApps,
                    PermissionEnum.HandlePayments,
                ],
            },
        });
        /**
        ...
        expect(webhook.secretKey).toEqual(
            appInDatabase.webhooks[0].secret!.secret,
        );
        expect(webhook.targetUrl).toEqual(
            `${env.require(
                "ECI_BASE_URL_FROM_CONTAINER",
            )}/api/saleor/webhook/v1/${appInDatabase.webhooks[0].id}`,
        );
    }, 60_000);
});
