...
export class XentralProxyOrderSyncService {
    ...
    private async versandArt(carrier: Carrier) {
        ...
        if (this.xentralCarriers.length > 0) {
            return (
                this.xentralCarriers.find((x) => x.eciCarrier === carrier)
                    ?.name || carrier
            );
        }
        return carrier;
    ...
    public async syncFromECI(): Promise<void> {
        ...
        try {
            ...
            for (const order of orders) {
                ...
                try {
                    const xentralAuftraegeWithSameDate =
                    ...
                    if (xentralAuftraegeWithSameOrderNumber.length === 0) {
                        ...
                        if (
                            xentralParentAuftraegeWithSameOrderNumber.length ===
                            0
                        ) {
                            this.logger.error(
                            ...
                            xentralParentAuftraegeWithSameOrderNumber.length > 1
                        ) {
                            this.logger.error(
                ...
                const auftrag: AuftragCreateRequest = {
                    ...
                    adresszusatz:
                        order.shippingAddress?.additionalAddressLine || "",
                    ...
                    artikelliste: {
                        position: order.orderLineItems.map((lineItem) => {
                            const price =
                                lineItem.totalPriceGross ||
                                lineItem.totalPriceNet;

                            // const encodedPrice = price
                            //   ? price.toString().replace(",", "").replace(".", ",")
                            //   : undefined;
                            const encodedPrice = price
                                ? (price / (lineItem?.quantity ?? 1))?.toFixed(
                                      2,
                                  )
                                : undefined;

                            if (
                                !lineItem?.productVariant?.xentralArtikel?.[0]
                                    ?.xentralNummer
                            ) {
                                throw new Error(
                            ...
                            return {
                                id: lineItem.productVariant.xentralArtikel[0]
                                    .id,
                                nummer: lineItem.productVariant
                                    .xentralArtikel[0].xentralNummer,
                ...
                if (
                    ...
                    throw new Error(
                        "Can not sync an Auftrag with an empty artikelliste",
                    );
                }
                ...
                const createOrUpdateXentralAuftrag = async () => {
                    if (existingXentralAuftrag) {
                        ...
                        const auftragUpdate: AuftragEditRequest = {
                            ...
                            kundennummer: parseInt(
                                existingXentralAuftrag.kundennummer,
                            ),
                ...
                const createdXentralAuftrag =
                    await this.db.xentralProxyAuftrag.upsert({
                        where: {
                ...
                this.logger.info(
                        ...
                        : "Created new xentralProxyAuftrag for current order",
                    {
                        ...
                        xentralAuftragBelegNr:
                            createdXentralAuftrag.xentralBelegNr,
                    },
                );
            }
        } catch (error) {
