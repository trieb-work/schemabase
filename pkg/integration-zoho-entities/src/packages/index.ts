...
export class ZohoPackageSyncService {
    ...
    public async syncToECI(): Promise<void> {
        ...
        if (cronState.lastRun === null) {
            ...
            this.logger.info(
                `Setting GTE date for package creation to ${gteDate}`,
            );
        }
        ...
        for (const parcel of packages) {
            ...
            if (
                 ...
                 */
                const fullPackage = await this.zoho.package.get(
                    parcel.package_id,
                );
                ...
                for (const lineItem of fullPackage.line_items) {
                    ...
                    const upsertedLineItem =
                        await this.db.packageLineItem.upsert({
                            where: {
    ...
    public async syncFromECI(): Promise<void> {
        ...
        for (const p of packagesNotInZoho) {
            try {
                if (
                    !p.trackingId &&
                    !["PICKUP", "UNKNOWN"].includes(p.carrier)
                ) {
                    this.logger.warn(
                        // eslint-disable-next-line max-len
        ...
        for (const p of packagesAlreadyDelivered) {
            ...
            try {
                await this.zoho.package.markDelivered(
                    zohoShipmentId,
                    new Date(),
                );
            } catch (error) {
