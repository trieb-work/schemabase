    ...
    public async syncToECI(): Promise<void> {
        ...
        for (const salesorder of salesorders) {
            ...
            const language = this.parseLanguage(
                (salesorder?.[
                    CustomFieldApiName.PREFERRED_LANGUAGE
                ] as string) ?? undefined,
            );
            ...
            const carrier = shippingMethodMatch(
                salesorder.delivery_method as string,
            );

            const readyToFullfill =
                (salesorder?.cf_ready_to_fulfill_unformatted as boolean) ||
                false;
            ...
            const trackingNotificationsEnabled = this.zohoApp
                ...
                ? parseBoolean(
                      salesorder?.[
                          this.zohoApp.customFieldTrackingNotifications
                      ],
                  )
                : undefined;
            ...
            if (
                ...
                try {
                    ...
                    const oldOrderLines =
                        createdSalesOrder.order.orderLineItems;
                ...
                try {
                    for (const payment of fullSalesorder.payments) {
                        const zohoPaymentExist =
                            await this.db.zohoPayment.findUnique({
                                where: {
                                    id_zohoAppId: {
                ...
                const contactPerson =
                    fullSalesorder.contact_person_details?.[0];
        ...
        for (const order of ordersFromEciDb) {
            try {
                ...
                if (!order.billingAddress || !order.shippingAddress) {
                    throw new Error(
                        "Billing and Shipping address need both to be set!",
                    );
                }
                ...
                const createdSalesOrder =
                    await this.zoho.salesOrder.create(createSalesOrderBody);
                await this.db.zohoSalesOrder.create({
                    data: {
                        ...
                        updatedAt: new Date(
                            createdSalesOrder.last_modified_time,
                        ),
                ...
                if (err instanceof Warning) {
                    ...
                    if ((err as ZohoApiError).code === 36004) {
                        ...
                        const searchedSalesOrders =
                            await this.zoho.salesOrder.search(
                                order.orderNumber,
                            );
                        ...
                        await this.db.zohoSalesOrder.create({
                            data: {
                                ...
                                createdAt: new Date(
                                    matchingSalesOrder.created_time,
                                ),
                                updatedAt: new Date(
                                    matchingSalesOrder.last_modified_time,
                                ),
