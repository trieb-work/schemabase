...
describe("Zoho Inventory SalesOrders Sync from internal ECI DB", () => {
    ...
    afterAll(async () => {
        if (CLEANUP_ORDERS) {
            ...
            await deleteOrders(prismaClient, {
                startsWith: ORDERNR_DATE_PREFIX,
            });

            const zohoPaymentIds = (await zoho.payment.list({}))
                .filter((pay) =>
                    pay.reference_number.startsWith(ORDERNR_DATE_PREFIX),
                )
            ...
            const zohoInvoiceIds = (await zoho.invoice.list({}))
                .filter((so) =>
                    so.reference_number.startsWith(ORDERNR_DATE_PREFIX),
                )
            ...
            console.log(
                ...
                await zoho.salesOrder
                    ...
                    .catch((err) =>
                        console.error(
                            "SalesOrder Cleanup failed: " + err.message,
                        ),
                    ),
            );
        }
