...
export class WorkflowScheduler {
  ...
  public async schedule(
    ...
    this.jobs.push(await queue.add(
      workflow.name,
      "RUN",
      {
        jobId: queueName,
  ...
  public async shutdownAllWorkers(){
    for(const worker of this.workers){
      this.logger.info("Shutting down worker", { workerName: worker.name });
      await worker.close();
  ...
  public async shutdownAllQueueSchedulers(){
    for(const queueScheduler of this.queueSchedulers){
      this.logger.info("Shutting down queuescheduler", { queueSchedulerName: queueScheduler.name });
      await queueScheduler.close();
      await queueScheduler.disconnect();
      this.logger.info("queuescheduler is offline", { queueSchedulerName: queueScheduler.name });
    }
  }
  ...
  public async removeAllJobs(){
    for(const job of this.jobs){
      ...
      try{
        ...
        this.logger.error("job is not removed", { err, jobName: job.name });
      }
    }
  }
}
