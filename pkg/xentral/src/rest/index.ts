...
export class XentralRestClient {
  ...
  constructor(config: { username: string; password: string; url: string; projectId?: number; authType?: 'digest' | 'basic'}) {
    this.url = config.url;
    this.projectId = config.projectId;
    switch(config.authType){
      case "basic": {
        // TODO: basic case is untested, not sure if it works
        const wrappedFetch = (url: RequestInfo, init?: RequestInit | undefined): Promise<Response> => {
          if(!init) {
            init = {};
          }
          init.headers = {
  ...
  public async apiFetch<Res, Req extends object = {}>(json: Req | null, subroute: string, method: AllowedMethod, params?: Record<string, string | number | boolean>, headers: HeadersInit = DEFAULT_HEADERS): Promise<Res> {
    const newParams = this.projectId ? { projekt: this.projectId, ...params, } : params;
    const queryString = (newParams && Object.keys(newParams).length > 0) ? `?${new URLSearchParams(newParams as Record<string, string>).toString()}` : '';
  ...
  public async *paginatedApiFetch<InnerRes, Res extends PaginatedRes<InnerRes> = PaginatedRes<any>, Req extends object = {}>(json: Req | null, subroute: AllowedSubRoutes, method: AllowedMethod, params?: Record<string, string | number | boolean>, items: number = DEFAULT_ITEM_COUNT, headers: HeadersInit = DEFAULT_HEADERS): AsyncIterableIterator<InnerRes> {
    let res: Res
    do {
  ...
  public async *getArtikel(params?: Record<string, string | number | boolean>, items: number = DEFAULT_ITEM_COUNT, headers: HeadersInit = DEFAULT_HEADERS) {
    yield* this.paginatedApiFetch<Artikel>(null, "/v1/artikel", "GET", params, items, headers);
  }
