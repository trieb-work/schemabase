...
export async function createZohoClient(zohoAppId: string, prisma: PrismaClient): Promise<Zoho> {
    const zohoApp = await prisma.zohoApp.findUnique({
        where: {
            id: zohoAppId,
        }
    })
    if (!zohoApp) throw new Error(`Could not find zoho app with provided id ${zohoAppId}`);
...
export async function prismaZohoAppEntryToClient(zohoApp: ZohoApp): Promise<Zoho> {
