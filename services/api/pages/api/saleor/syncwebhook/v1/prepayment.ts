    ...
    if (isPaymentWebhook) {
        ...
        if (saleorEvent === "payment_list_gateways") {
            const paymentGateways =
                await vorkassePaymentService.paymentListGateways("EUR");
            console.info(
        ...
        if (saleorEvent === "payment_confirm") {
            ...
            console.info(
                "Payment confirm request. Responding with confirmation",
            );
            return jsonResponseOk(response);
        }
        if (saleorEvent === "payment_capture") {
            ...
            console.info(
                "Payment capture request. Responding with confirmation",
            );
            return jsonResponseOk(response);
        }
